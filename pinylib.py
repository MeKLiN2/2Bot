...
#python2 bot.py | espeak
    ...
    def on_niglist(self, banlist_info):

        if not niglist_info['success']:
            self.console_write(COLOR['bright_red'], banlist_info['reason'])
        else:
            if len(niglist_info['items']) > 0:
                for item in niglist_info['items']:
                    self.users.add_nig_user(item)
            else:
                self.console_write(COLOR['green'], 'The banlist is empty.')
