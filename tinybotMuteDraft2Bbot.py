...
#### 1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l
    ...
    def message_handler(self, msg):
        ...
        if "windows" in msg:
            self.send_chat_msg("%s! https://archive.org/details/7700legends the fastest version of windows ever made for high ranked, serious players, or tournament computers. this is the only windows .iso with zero dpc latency on the cpu. lovingly hand-edited by MeKLiN on 06-17-2022" % self.active_user.nick)

        if "ignoreadd" in msg:
            sleep(randint(1,3))
            self.send_chat_msg("%s's computer is hacked by CTS script! Read more at: https://pastebin.com/1eJZ2hEG" % self.active_user.nick)
            sleep(randint(1,3))
            self.send_chat_msg('https://imgur.com/a/GACGIn9 open source script for ublock origins by meklin, blocks ALL javascript for in a chat, but not on the main TC page')
            sleep(randint(1,3))
            self.send_chat_msg('https://pastebin.com/TiDCRpPH')
        ...
        if msg.startswith(prefix):
            ...
            if self.has_level(5):
                ...
                if cmd == prefix + 'acspy':
                   ...
                   #webbrowser.get('firefox').open_new_tab('%s' % cmd_arg)
