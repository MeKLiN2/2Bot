...
#### 1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l
    ...
    def message_handler(self, msg):
        ...
        if "ignoreadd" in msg:
            sleep(randint(1,10))
            self.send_chat_msg("%s's computer is hacked by CTS script! They were removed for compromising the website and all of us users. Read more at: https://pastebin.com/1eJZ2hEG" % self.active_user.nick)
            sleep(randint(10,20))
            self.send_chat_msg('https://greasyfork.org/en/scripts/389657-tinychat-enhancement-suite-tes/code')
            sleep(randint(20,30))
            self.send_chat_msg('https://www.tampermonkey.net')
            #sleep(randint(30,40))
            #self.send_chat_msg('!acc holden %s' % self.active_user.account)
            #sleep(randint(40,60))
            #self.send_chat_msg('Say bye-bye, %s...' % self.active_user.account)
            #sleep(randint(10,15))
            #self.send_chat_msg('Dumb motherfucker.')
            #sleep(randint(4,8))
            #self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "script" in msg:
            sleep(randint(3,6))
            self.send_chat_msg("Psst... hey %s" % self.active_user.nick)
            sleep(randint(8,14))
            self.send_chat_msg("Want MeKLiN's personal user script, %s? https://pastebin.com/raw/H9p5jK4b" % self.active_user.account)
            sleep(randint(4,6))
            self.send_chat_msg('https://addons.mozilla.org/en-US/firefox/addon/ublock-origin/')
            sleep(randint(4,6))
            self.send_chat_msg('https://chrome.google.com/webstore/detail/ublock-origin/cjpalhdlnbpafiamejdnhcphjbkeiagm?hl=en')
            sleep(randint(4,6))
            self.send_chat_msg('Add the script into the My Filters tab in the Dashboard of UBlock Origins. Works good with https://greasyfork.org/en/scripts/389657-tinychat-enhancement-suite-tes/code')
            sleep(randint(4,6))
            self.send_chat_msg('And remember, the CTS script is compromised. Anything can happen to your computer if malicious javascript is run in your browser. Anything. Read and learn. Share. https://pastebin.com/1eJZ2hEG')
        ...
        if len(msg) > 550:
            ...
            #self.do_bad_nick(self.active_user.nick)
            #self.send_chat_msg('!acc ban %s' % self.active_user.account)
            self.send_chat_msg("You have earned ONE spam point! (%s)" % len(msg))
            #sleep(randint(60,120))
            #self.send_chat_msg("Don't foget, you earned (%s) spampoints. Type 'spendpoints' to buy something." % len(msg))
	    ...
	    #admin/botowner
            ...
            if self.has_level(5):
                ...
                elif cmd == prefix + "nigger":
                    self.send_chat_msg('Initiating Nigger Check for %s' % cmd_arg)
                    sleep(randint(1,2))
